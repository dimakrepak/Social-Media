...
export default function Rightbar({ profile }) {
  ...
  const ProfileRightbar = () => {
    ...
    const [followed, setFollowed] = useState(
      currentUser.user.following.includes(profile?._id)
    );
    ...
    return (
      <>
        ...
        <div className="profile-rightbar__intro">
          {editMode ? <h2>Edit Intro</h2> : <h2>Intro</h2>}
          ...
          {currentUser.user._id === profile._id && (
            <button
              className="profile-rightbar-info__edit"
          ...
          {editMode && (
            <button
              className="profile-rightbar-info__save"
        ...
        <div className="profile-rightbar-friends">
          <h2>Friends</h2>
          <span className="profile-rightbar-friends-amount">
            ...
            {friends &&
              friends.map((friend) => (
                <div className="profile-friend_item" key={friend._id}>
                  ...
                  <span className="profile-friend_item-username">
                    {friend.username}
                  </span>
                </div>
              ))}
